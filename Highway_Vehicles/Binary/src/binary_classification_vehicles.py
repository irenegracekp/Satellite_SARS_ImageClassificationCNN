"""for binary classification of vehicles on highway"""
import copy
import torch
...
trainDataset = datasets.ImageFolder(root='./inputs/Images', transform=transformOri)
...
class CNN(nn.Module):
	...
	def __init__(self):
		...
		self.fc6 = nn.Sequential(nn.Linear(8192, 2000), nn.ReLU(), nn.Dropout(0.5), nn.Linear(2000, 500), nn.ReLU(), nn.Dropout(0.4), nn.Linear(500, 125), nn.ReLU(), nn.Dropout(0.3), nn.Linear(125, 30), nn.ReLU(), nn.Dropout(0.2), nn.Linear(30, 7), nn.Dropout(0.1), nn.Linear(7, 2), nn.Softmax(dim=1))
